...
type MessagePart = AgentMessage["speaking"]["parts"][number];

interface EvaluateSpeakingToolProps {
	toolPart: Extract<MessagePart, { type: "tool-evaluate-speaking" }>;
}

export function EvaluateSpeakingTool({ toolPart }: EvaluateSpeakingToolProps) {
	...
	if (toolPart.state === "output-error") {
		return (
			<Task
				...
				label="Evaluate speaking"
			>
				<ToolErrorContent
					description={toolPart.errorText}
					title="Failed to evaluate speaking"
					toolPart={toolPart}
				/>
			</Task>
		);
	...
	const label = overallBand
		? `Speaking evaluated: Band ${overallBand}`
							...
							{(toolPart.state === "input-available" || isComplete) && (
								<div className="flex flex-wrap gap-1.5">
									<Badge className="text-xs" variant="outline">
										FC: {toolPart.input.fluencyCoherence}
									</Badge>
									<Badge className="text-xs" variant="outline">
									...
									<Badge className="text-xs" variant="outline">
										P: {toolPart.input.pronunciation}
									</Badge>
								</div>
							)}
						</>
