import { useMutation } from "@tanstack/react-query";
import { getDefaultStore } from "jotai";
import { createListeningChatMutationOptions } from "#./lib/create-listening-chat-mutation-options.ts";
import { ListeningPromptInput } from "#./lib/listening-prompt-input.tsx";
import { PageHeader } from "#./lib/page-header.tsx";
import { sidebarOpenAtom } from "#./lib/sidebar-open-atom.ts";
import { SuggestionPrompts } from "#./lib/suggestion-prompts.tsx";
import type { PromptInputMessage } from "~/components/ai-elements/prompt-input";
import {
	PromptInputProvider,
	usePromptInputController,
} from "~/components/ai-elements/prompt-input";
import {
	Breadcrumb,
	BreadcrumbItem,
	BreadcrumbList,
	BreadcrumbPage,
} from "~/components/ui/breadcrumb";

const LISTENING_SUGGESTIONS = [
	"A phone call to book a hotel room",
	"A university orientation tour guide speech",
	"Students discussing a research project on renewable energy",
	"A lecture on the impact of climate change on marine biodiversity",
];

export function clientLoader() {
	const store = getDefaultStore();
	store.set(sidebarOpenAtom, true);
}

function ListeningContent({
	isSubmitting,
	onSubmit,
}: {
	isSubmitting: boolean;
	onSubmit: (message: PromptInputMessage) => Promise<void> | void;
}) {
	const { textInput } = usePromptInputController();

	return (
		<>
			<h1 className="mb-2 text-3xl font-bold">IELTS Listening Test</h1>
			<p className="mb-8 max-w-md text-center text-sm text-muted-foreground">
				Practice IELTS listening tests with audio on any topic
			</p>
			<div className="w-full max-w-xl">
				<ListeningPromptInput
					externalProvider
					disabled={isSubmitting}
					placeholder="Describe the topic for your IELTS listening test..."
					onSubmit={onSubmit}
				/>
			</div>
			<SuggestionPrompts
				disabled={isSubmitting}
				suggestions={LISTENING_SUGGESTIONS}
				onSelect={(suggestion) => {
					textInput.setInput(suggestion);
				}}
			/>
			<div className="h-28" />
		</>
	);
}

export default function Component() {
	const createMutation = useMutation(createListeningChatMutationOptions);
	return (
		<div className="flex h-full flex-col">
			<PageHeader>
				<Breadcrumb>
					<BreadcrumbList>
						<BreadcrumbItem>
							<BreadcrumbPage>Listening</BreadcrumbPage>
						</BreadcrumbItem>
					</BreadcrumbList>
				</Breadcrumb>
			</PageHeader>
			<main className="flex flex-1 flex-col items-center justify-center px-4">
				<PromptInputProvider>
					<ListeningContent
						isSubmitting={createMutation.isPending}
						onSubmit={async (message) => {
							await createMutation.mutateAsync(message);
						}}
					/>
				</PromptInputProvider>
			</main>
		</div>
	);
}
